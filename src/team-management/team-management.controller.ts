...
export class TeamManagementController {
    ...
    @Post("/add")
    async addTeam(@Body() body:TeamBody, @Req() requist:FastifyRequest, @Res() reply:FastifyReply){
        try {
    ...
    @Post("/update")
    async updateTeam(@Body() {_id,...field}:TeamBody, @Req() request:FastifyRequest, @Res() reply:FastifyReply){
        try {
